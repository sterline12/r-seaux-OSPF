Réseau de communication via le protocole d’OSPF avec d’autres réseaux. 
...
Réaliser le routage pour la configuration selon le protocole OSPF, de sorte que le réseau forme un espace totalement connexe, c-à-d tout
terminal rattaché à tout routeur peut être joint par tout autre terminal du même réseau. De plus, ce réseau, qui est vu comme un AS,  
possède un Intranet et il est connecté au reste d’INTERNET, ainsi qu’à un site particulier via une liaison dédié. 
